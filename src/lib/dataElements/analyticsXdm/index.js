...
const numberedKeyMapper = (prefix, max, mapper) => (key, value, xdm) => {
  ...
  const i = parseInt(key.substring(prefix.length));
  if ( i > 0 && i <= max) {
    if (value) {
      mapper(i, value, xdm);
    }
...
const propMapper = delimiters => (key, value, xdm) => {
  ...
  if (delimiter) {
    ...
    setValue(xdm, `_experience.analytics.customDimensions.listProps.prop${key}`, { delimiter, values });
  } else {
    setValue(xdm, `_experience.analytics.customDimensions.props.prop${key}`, `${value}`);
  }
};
...
const hierMapper = delimiters => (key, value, xdm) => {
  ...
  const values = `${value}`.split(delimiter);
  setValue(xdm, `_experience.analytics.customDimensions.hierarchies.hier${key}`, { delimiter, values });
};

const listMapper = delimiters => (key, value, xdm) => {
  const list = `${value}`.split(delimiters[`list${key}`] || ",").map(item => ({ value: item }));
  setValue(xdm, `_experience.analytics.customDimensions.lists.list${key}`, list);
};

const evarMapper = (key, value, xdm) => {
  setValue(xdm, `_experience.analytics.customDimensions.eVars.eVar${key}`, `${value}`);
};
